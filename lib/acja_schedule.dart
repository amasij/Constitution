...
final pageName='Acja Schedules';
TextEditingController searchController = TextEditingController();

class AcjaSchedule extends StatefulWidget {
 static String get routeName => '/acjaSchedule';
  @override
  AcjaScheduleState createState() => AcjaScheduleState();
}
...
class AcjaScheduleState extends State<AcjaSchedule> with AutomaticKeepAliveClientMixin {
  @override
  ...
  loadChapterJson() async {
    return await DefaultAssetBundle.of(context)
        .loadString('assets/acja_schedule_chapters.json');
  }

  loadSectionJson() async {
    return await DefaultAssetBundle.of(context)
        .loadString('assets/acja_schedule_sections.json');
  }

  loadPartJson() async {
    return await DefaultAssetBundle.of(context)
        .loadString('assets/acja_schedule_parts.json');
  }

  loadContentJson() async {
    return await DefaultAssetBundle.of(context)
        .loadString('assets/acja_schedule_contents.json');
  }
          ...
          Expanded(
              ...
              ListView.builder(
                ...
                itemBuilder: (BuildContext context, int index) 
                {
                  ...
                  return AcjaScheduleEntryItem(mainData[index]);
                },
...
String getHeader(int sectionId)
{
  ...
  var chapter=getChapter(section['chapter'], chapterSearchList);
...
class AcjaScheduleEntryItem extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
  ...
  const AcjaScheduleEntryItem(this.entry);
      ...
      if (searchSection) {
        ...
        for (var i = 0; i < contentData.length; i++) {
          if (contentData[i]['content']
            ...
            var chapter = chapterSearchList[section['chapter']];
